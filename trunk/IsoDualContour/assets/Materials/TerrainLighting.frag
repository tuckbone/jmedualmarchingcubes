...
vec2 computeLighting(in vec3 wvPos, in vec3 wvNorm, in vec3 wvViewDir, in vec3 wvLightDir){
  ...
  vec3 calculateNormal(in vec2 texCoord) {
    ...
    vec4 calculateTriPlanarDiffuseBlend(in vec3 wNorm, in vec4 wVert, in vec2 texCoord) {
        ...
        float norXY = abs(wNorm.z);
        float norXZ = abs(wNorm.y);
        float norYZ = abs(wNorm.x);  
        float total = (norXY + norYZ + norXZ);
        norXY /= total;
        norXZ /= total;
        norYZ /= total;

        vec4 texXY = texture2D(m_DiffuseMap, wVert.xy*m_DiffuseMap_0_scale);
     //   vec4 texXZ = texture2D(m_DiffuseMap, wVert.xz*m_DiffuseMap_0_scale);
     //   vec4 texYZ = texture2D(m_DiffuseMap, wVert.yz*m_DiffuseMap_0_scale);
        result = (texXY * norXY );// +  texXZ * norXZ + texYZ * norYZ); 

        vec4 texXZ = texture2D(m_DiffuseMap_1, wVert.xz*m_DiffuseMap_1_scale);
        result += (texXZ * norXZ); 

        vec4 texYZ = texture2D(m_DiffuseMap_2, wVert.yz*m_DiffuseMap_2_scale);  
        result += (texYZ * norYZ); 
