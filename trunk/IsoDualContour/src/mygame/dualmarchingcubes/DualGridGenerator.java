...
public class DualGridGenerator {
    ...
    private LinkedList<DualCell> dualCells = new LinkedList();
    
    private IsoSurface is;
    private MeshBuilder mb;
    
    private boolean saveDualCells;
    
    /// The maximum distance where to generate the skirts.
    private float maxMSDistance;
     
    private Vector3f totalFrom;
    private Vector3f totalTo;
    ...
    public class DualCell
    {
        ...
        public DualCell(Vector3f c0, Vector3f c1, Vector3f c2, Vector3f c3, 
            Vector3f c4, Vector3f c5, Vector3f c6, Vector3f c7)
        {
            c = new Vector3f[8];
            c[0] = c0;
            c[1] = c1;
   ...
   public void addDualCell(Vector3f c0, Vector3f c1, Vector3f c2, Vector3f c3, 
            Vector3f c4, Vector3f c5, Vector3f c6, Vector3f c7)
        {
    ...
    public void addDualCell(Vector3f c0, Vector3f c1, Vector3f c2, Vector3f c3, 
            Vector3f c4, Vector3f c5, Vector3f c6, Vector3f c7, float values[], Vector3f gradients[])
        {

            if (saveDualCells)
            {
                dualCells.add(new DualCell(c0, c1, c2, c3, c4, c5, c6, c7));
            }
            ...
            corners[6] = c6;
            corners[7] = c7;
            is.addMarchingCubesTriangles(corners, values,gradients, mb);
            
            
            Vector3f from = root.getFrom();
            Vector3f to = root.getTo();
            if (corners[0].z == from.z && corners[0].z != totalFrom.z)
            {
                is.addMarchingSquaresTriangles(corners, values, gradients, IsoSurface.MS_CORNERS_BACK, maxMSDistance, mb);
            }
            if (corners[2].z == to.z && corners[2].z != totalTo.z)
            {
                is.addMarchingSquaresTriangles(corners, values,gradients, IsoSurface.MS_CORNERS_FRONT, maxMSDistance, mb);
            }
            if (corners[0].x == from.x && corners[0].x != totalFrom.x)
            {
                is.addMarchingSquaresTriangles(corners, values,gradients, IsoSurface.MS_CORNERS_LEFT, maxMSDistance, mb);
            }
            if (corners[1].x == to.x && corners[1].x != totalTo.x)
            {
                is.addMarchingSquaresTriangles(corners, values,gradients, IsoSurface.MS_CORNERS_RIGHT, maxMSDistance, mb);
            }
            if (corners[5].y == to.y && corners[5].y != totalTo.y)
            {
                is.addMarchingSquaresTriangles(corners, values,gradients, IsoSurface.MS_CORNERS_TOP, maxMSDistance, mb);
            }
            if (corners[0].y == from.y && corners[0].y != totalFrom.y)
            {
                is.addMarchingSquaresTriangles(corners, values,gradients, IsoSurface.MS_CORNERS_BOTTOM, maxMSDistance, mb);
            }
        }
    
    
    public void generateDualGrid(OctreeNode root, IsoSurface is, MeshBuilder mb, float maxMSDistance, Vector3f totalFrom, Vector3f totalTo, boolean saveDualCells)
    {
        this.root = root;
        this.is = is;
        this.mb = mb;
    ...
    public List<DualCell> getDualCells()
    {
        return dualCells;
    }
}
