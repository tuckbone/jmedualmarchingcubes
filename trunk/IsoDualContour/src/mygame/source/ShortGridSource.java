 ...
 * @author Karsten
 */
public class ShortGridSource implements VolumeSource {
    ...
    private short maxClampedAbsoluteDensity = 0;
    ...
    public ShortGridSource(Vector3f scale, int width, int height, int depth)
    {
        this.scale=scale;
    ...
    public void setVolumeGridValue(int x, int y, int z, short value) {
        // Clamp if wanted.
        if (maxClampedAbsoluteDensity != 0) {
            if (value > maxClampedAbsoluteDensity) {
                ...
                value = (short) (-maxClampedAbsoluteDensity);
            }
        }
    ...
    public short getVolumeGridValue(int x, int y, int z) {
        if (x >= width) {
            x = width - 1;
        } else if (x < 0) {
            x = 0;
